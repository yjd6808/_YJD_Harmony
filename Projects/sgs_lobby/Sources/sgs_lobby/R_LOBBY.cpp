...
void R_LOBBY::RECV_CLO_JoinLobby(Session* _pSession, ICommand* _pCmd)
{
	...
	if (!sg::Contents.UnauthenticatedSessionManager->Add(pCmd->Serial, pSession)) {
	...
	S_AUTHENTICATION_IS::SetInformation(sg::InterServerClientTcp, SendStrategy::SendAsync, SingleServerType::Auth);
