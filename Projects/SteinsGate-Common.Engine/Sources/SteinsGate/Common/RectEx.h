...
public:
	 ...
	 * 하단 테스트코드 참고
	 */
	static bool Intersect(const SGRect& _rc, const SGRect& _rc2, SGRect& _intersectRect);
	static void Log(const SGRect& _rc);
	...
	*/
	...
	static bool IntersectY(const SGRect& _lhs, const SGRect& _rhs);

	// rc의 가로길이와 pos의 x좌표가 겹치는지
	static bool ContainsX(const SGRect& _rc, const float _x);

	// rc의 세로길이와 pos의 y좌표가 겹치는지
	static bool ContainsY(const SGRect& _rc, const float _y);
