...
CenterSession::CenterSession(
	...
	, m_eClientType(CenterClientType::None)
...
void CenterSession::SetBootState(ServerBootState_t state) {
	...
	if (state == ServerBootState::Launched) {
		_LogInfo_("%s 서버가 실행되었습니다.", CenterClientType::Name[m_eClientType]);
	} else if (state == ServerBootState::Stopped) {
		_LogInfo_("%s 서버가 중지되었습니다.", CenterClientType::Name[m_eClientType]);
	}
}
