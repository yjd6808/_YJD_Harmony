...
inline constexpr float CharacterPartFrameContentWidth_v = 500.0f;
inline constexpr float CharacterPartFrameContentHeight_v = 600.0f;

inline constexpr float CharacterSpriteFrameDelay_v = 0.1f;	// 0.1초

inline constexpr float CharacterBoundingBoxWidth_v = 50.0f;
inline constexpr float CharacterBoundingBoxHeight_v = 120.0f;
...
inline constexpr JCore::Tuple<int, int> CharacterMotionDefailIndexPairs_v[]
{
...
inline auto MotionDetail_TypeMapInitializer = []()->int {
	for (int i = 0; i < MotionState::Max; ++i) {
		const int iBegin = CharacterMotionDefailIndexPairs_v[i].item1;
		const int iEnd = CharacterMotionDefailIndexPairs_v[i].item2;
