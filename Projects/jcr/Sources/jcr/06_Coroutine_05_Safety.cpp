...
static void PrintSection05(const char* _pName)
{
...
{
	PrintSection05("CO05-1: 다른 스레드에서 resume → coeWrongThread");
	...
	std::thread th([&]
	{
		...
		if (r == nullptr && CoGetLastError() == coeWrongThread)
			Console::WriteLine(ConsoleColor::Green, "  PASS [CO05-1] 타 스레드 resume 거부");
		else
			Console::WriteLine(ConsoleColor::Red, "  FAIL [CO05-1] 막지 못함");
	});
	th.join();

	if (!g_ran05)
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO05-1] 본문 미실행 확인");
	else
		Console::WriteLine(ConsoleColor::Red, "  FAIL [CO05-1] 본문이 실행됨");
...
static void fn_Co05_Self(CoContext*)
{
	...
	if (r == nullptr && CoGetLastError() == coeInvalidState)
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO05-2] 자기 resume 거부");
	else
		Console::WriteLine(ConsoleColor::Red, "  FAIL [CO05-2] 막지 못함");
}

static void Test_Co05_Self()
{
	PrintSection05("CO05-2: 자기 자신을 resume → coeInvalidState");
...
{
	PrintSection05("CO05-3: 재사용된 주소 + 예전 핸들 → coeStaleHandle");
	...
	if (hB.pCtx == h.pCtx)
		Console::WriteLine(ConsoleColor::Cyan, "    [CO05-3] 같은 주소 재사용 확인: %p", h.pCtx);

	CoContext* r = CoResumeH(h);
	if (r == nullptr && CoGetLastError() == coeStaleHandle)
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO05-3] 예전 핸들 거부");
	else
		Console::WriteLine(ConsoleColor::Red, "  FAIL [CO05-3] 막지 못함");
