...
// VEH 자체 방어 예제.
// - VEH 안에서 나는 폴트는 디버깅이 거의 불가능하고,
//   재진입 깊이 초과나 남의 가드 오판이 나면 조용히 망가진다.
...
static void Test_Co15_Stats()
{
	...
	CoId id = CoRun(fn_Co15_Grow);
	while (CoResume(id)) {}

	CoMgr::CoVehStats st = g_cCoMgr.GetVehStats();
	Console::WriteLine(ConsoleColor::Cyan,
		_T("    [CO15] maxDispatchUsed=%llu minRemain=%lld"),
		(unsigned long long)st.maxDispatchUsed_, (long long)st.minRemain_);
	if (st.maxDispatchUsed_ == 0 && st.minRemain_ == (size_t)-1)
		Console::WriteLine(ConsoleColor::Cyan,
			_T("    [CO15] 우리 VEH가 안 불림 (커널이 성장을 직접 처리함)"));
	else if ((long long)st.minRemain_ >= 2048)
