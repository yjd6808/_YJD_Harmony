...
static void PrintSection15(const char* _pName)
{
...
static void Test_Co15_Stats()
{
	...
	Console::WriteLine(ConsoleColor::Cyan,
		"    [CO15] maxDispatchUsed=%llu minRemain=%lld",
		(unsigned long long)st.maxDispatchUsed, (long long)st.minRemain);
	if (st.maxDispatchUsed == 0 && st.minRemain == (size_t)-1)
		Console::WriteLine(ConsoleColor::Cyan,
			"    [CO15] 우리 VEH가 안 불림 (커널이 성장을 직접 처리함)");
	else if ((long long)st.minRemain >= 2048)
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO15] 가드 여유 충분");
	else
		Console::WriteLine(ConsoleColor::Yellow, "  WARN [CO15] 가드 여유 부족. 예산 상향 검토");
}
