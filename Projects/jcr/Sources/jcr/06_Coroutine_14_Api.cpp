...
// API 편의성 예제. (userData / 값 채널 / 람다 / CoScope)
...
static void Test_Co14_UserData()
{
	...
	int acc = 0;
	CoId id = CoRun(fn_Co14_UserData, {.userData_ = &acc});
	while (CoResume(id)) {}
	if (acc == 11)
...
static void Test_Co14_Transfer()
{
	...
	CoId id = CoRun(fn_Co14_Echo, {.ppOut_ = &pView});
	Console::WriteLine(ConsoleColor::Cyan, _T("    [CO14-2] 첫 yield: %llu (기대 100)"),
		(unsigned long long)pView->switchData_);
	_u64 out = 0;
	CoResume(id, (_u64)111, &out);
...
{
	PrintSection14(_T("CO14-4: CoScope 협력적 취소"));

	{
		CoScope sc(CoRun(fn_Co14_Cancel));
		sc.Resume();
		sc.Cancel();
		if (!CoResume(sc.Id()))
