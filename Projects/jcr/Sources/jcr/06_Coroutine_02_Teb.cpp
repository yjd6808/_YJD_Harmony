...
static void fn_Co02_ReadTeb(CoContext* pCtx)
{
	...
	Console::WriteLine(ConsoleColor::Cyan, _T("    [CO02] TEB 안: Base=0x%p Limit=0x%p Dealloc=0x%p"),
		base, limit, dealloc);
	Console::WriteLine(ConsoleColor::Cyan, _T("    [CO02] 스택   : Base=0x%p End =0x%p"),
		pCtx->stack_.pStackBase_, pCtx->stack_.pStackEnd_);
	if (base == pCtx->stack_.pStackBase_)
		Console::WriteLine(ConsoleColor::Green, _T("  PASS [CO02] StackBase 일치"));
	else
		Console::WriteLine(ConsoleColor::Red, _T("  FAIL [CO02] StackBase 불일치"));
}
