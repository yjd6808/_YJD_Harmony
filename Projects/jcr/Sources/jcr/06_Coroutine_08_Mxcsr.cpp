...
static void fn_Co08_Rounding(CoContext*)
{
	...
	Console::WriteLine(ConsoleColor::Cyan, _T("    [CO08] 코루틴: 내림 모드 설정 후 yield"));
	CoYield();
	unsigned now = 0;
	_controlfp_s(&now, 0, 0);
	if ((now & _MCW_RC) == _RC_DOWN)
		Console::WriteLine(ConsoleColor::Green, _T("  PASS [CO08] 재개 후에도 내림 모드 유지"));
	else
		Console::WriteLine(ConsoleColor::Red, _T("  FAIL [CO08] 모드 유실"));
}
...
static void Test_Co08_Rounding()
{
	...
	if ((mid & _MCW_RC) == _RC_NEAR)
		Console::WriteLine(ConsoleColor::Green, _T("  PASS [CO08] 스레드 쪽은 그대로(최근접)"));
	else
