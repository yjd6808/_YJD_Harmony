...
static void PrintSection07(const char* _pName)
{
...
{
	PrintSection07("CO07-1: CoRun(null) → coeNullFunction");

	CoContext* pCtx = CoRun(nullptr, cstMid);
	if (pCtx == nullptr && CoGetLastError() == coeNullFunction)
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO07-1] %s", CoErrorString(coeNullFunction));
	else
		Console::WriteLine(ConsoleColor::Red, "  FAIL [CO07-1] 원인이 보이지 않음");
}
...
static void fn_Co07_Grow(CoContext*)
{
	...
	for (int i = 0; i < (int)sizeof(buf); i += 4096)
		buf[i] = (char)i;
	Console::WriteLine(ConsoleColor::Cyan, "    [CO07-2] 10KB 터치 완료 (VEH 자동 등록으로 확장됨)");
}

static void Test_Co07_AutoVeh()
{
	PrintSection07("CO07-2: VEH 미등록 상태에서 10KB 스택 확장");

	CoContext* pCtx = CoRun(fn_Co07_Grow, cstMid);
	if (pCtx == nullptr && CoGetLastError() == coeNone)
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO07-2] 확장 후 정상 종료");
	else
		Console::WriteLine(ConsoleColor::Red, "  FAIL [CO07-2] 확장 실패");
}
