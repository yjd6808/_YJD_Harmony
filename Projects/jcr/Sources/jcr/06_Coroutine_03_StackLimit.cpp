...
static void PrintSection03(const char* _pName)
{
...
static void fn_Co03_GrowThrow(CoContext* pCtx)
{
	...
	if (limitNow == pCtx->stack_.pStackLimit_)
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO03] TEB 한계 동기화됨");
	else
		Console::WriteLine(ConsoleColor::Red, "  FAIL [CO03] TEB 한계 불일치");

	try { throw 42; }
	catch (int v)
	{
		if (v == 42)
			Console::WriteLine(ConsoleColor::Green, "  PASS [CO03] 확장 영역에서 throw/catch");
		else
			Console::WriteLine(ConsoleColor::Red, "  FAIL [CO03] 값 불일치");
	}
}

static void Test_Co03_GrowThrow()
{
	PrintSection03("CO03: 16KB 확장 후 throw/catch + 한계 동기화");
