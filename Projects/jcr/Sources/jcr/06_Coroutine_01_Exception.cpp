...
//   스케줄러 스택에서 다시 던진다.
static void PrintSection01(const char* _pName)
{
...
{
	Console::WriteLine(ConsoleColor::Cyan, "    [CO01] fn 안에서 throw");
	throw std::runtime_error("코루틴 예외");
}

static void Test_Co01_Checked()
{
	PrintSection01("CO01-1: CoRunChecked → 스케줄러 스택에서 catch");

	try
	{
		CoContext* pCtx = CoRunChecked(fn_Co01_Throw, cstMid);
		Console::WriteLine(ConsoleColor::Cyan, "    [CO01-1] CoRun 반환: %p (종료됐으므로 null)", pCtx);
	}
	catch (const std::runtime_error& e)
	{
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO01-1] 받음: %s", e.what());
	}
...
{
	PrintSection01("CO01-2: CoRun(미검사) → 죽지 않고 끝나며 원인이 남음");

	CoContext* pCtx = CoRun(fn_Co01_Throw, cstMid);
	if (pCtx == nullptr && CoGetLastError() == coeException)
		Console::WriteLine(ConsoleColor::Green, "  PASS [CO01-2] 종료 + %s", CoErrorString(coeException));
	else
		Console::WriteLine(ConsoleColor::Red, "  FAIL [CO01-2] pCtx=%p", pCtx);
	CoClearPendingException();
}
