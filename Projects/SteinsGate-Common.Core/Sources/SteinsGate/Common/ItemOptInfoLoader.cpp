...
//////////////////////////////////////////////////////////////////////////////////////////
ItemOptInfoLoader::ItemOptInfoLoader(DataManagerAbstract* _pManager)
...
bool ItemOptInfoLoader::Load()
{
	...
	try
	{
		...
		for (int i = 0; i < itemOptListRoot.size(); ++i)
		{
			...
			DebugAssertMsg(levelOptCount > 0, "아이템 옵트 카운트가 0입니다.");
			...
			{
				DebugAssertMsg(false, "아이템 옵트 목록에 이미 %s 데이터가 존재하는데 넣을려고 했습니다.", pItemOpt->eName_.Source());
			}
		}
	}
...
//////////////////////////////////////////////////////////////////////////////////////////
ItemOptInfo* ItemOptInfoLoader::GetData(const SGString& _name)
{
	DebugAssertMsg(dataMapByName_.Exist(_name), "아이템 옵트 목록에 %s라는 옵트의 정보는 존재하지 않습니다.", _name.Source());
