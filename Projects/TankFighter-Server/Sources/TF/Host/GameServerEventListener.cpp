...
void GameServerEventListener::OnStarted() {
	SafeConsole::WriteLine(ConsoleColor::LightGray, "게임 서버 시작됨");
}

void GameServerEventListener::OnConnected(JNetwork::TcpSession* connectedSession) {
	SafeConsole::WriteLine(ConsoleColor::Cyan, "유저 : %s 접속", connectedSession->GetRemoteEndPoint().ToString().Source());
...
void GameServerEventListener::OnDisconnected(JNetwork::TcpSession* disconnetedSession) {
	SafeConsole::WriteLine(ConsoleColor::LightGray, "유저 : %s 접속 종료", disconnetedSession->GetRemoteEndPoint().ToString().Source());
	...
	if (_World->RemovePlayer(player)) {
		SafeConsole::WriteLine("월드에서 플레이어 정보 안전하게 제거완료");
	}
...
void GameServerEventListener::OnReceived(JNetwork::TcpSession* receiver, JNetwork::ICommand* cmd) {
	if (!m_pParser->RunCommand(receiver->GetTag<Player*>(), cmd)) {
		SafeConsole::WriteLine(ConsoleColor::Brown, "커맨드 파싱 실패(Command ID : %d)", cmd->GetCommand());
	}
}
...
void GameServerEventListener::OnStopped() {
	SafeConsole::WriteLine("서버가 종료되었습니다.");
}