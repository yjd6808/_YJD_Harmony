...
bool MysqlDatabase::Initialize() {
	...
	if (!m_pConnectionPool->Init(DB_CONN_POOL_SIZE)) {
		SafeConsole::WriteLine(ConsoleColor::LightGray, "DB 커넥션 풀 초기화 실패");
		return false;
	}
	...
	if (!MysqlStatementBuilder::Initialize()) {
		SafeConsole::WriteLine(ConsoleColor::LightGray, "DB 스테이트먼트 빌더 초기화 실패");
		return false;
	}

	SafeConsole::WriteLine(ConsoleColor::Green, "데이터베이스 커넥션 풀(%d)이 성공적으로 초기화되었습니다. [localhost/%d]", DB_CONN_POOL_SIZE, DB_PORT);
	...
	SafeConsole::WriteLine(ConsoleColor::Green, "데이터베이스 IOCP가 성공적으로 생서되었습니다.");
