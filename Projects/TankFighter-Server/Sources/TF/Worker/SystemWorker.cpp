...
void SystemWorker::WorkerThread() const {
	Console::WriteLine("SystemWorker 쓰레드가 실행되었습니다. (%d)", std::this_thread::get_id());
	...
	for (;;) {
		...
		if (std::cin >> com) {
			if (com == 'c' || com == 'C') {
				...
				Console::WriteLine(ConsoleColor::LIGHTBLUE, "[통합] : 인원 : %d명 / 방의 수 : %d개 / 게임중인 방의 수 : %d", 
					pWorld->GetTotalPlayerCount(), pWorld->GetTotalRoomCount(), pWorld->GetTotalPlayingRoomCount());

				for (int i = 0; i < channels.Size(); i++) {
					Console::WriteLine(ConsoleColor::GREEN, "[채널 %d] : 인원 : %d명 / 방의 수 : %d개 / 게임중인 방의 수 : %d", 
...
THREAD_END:
	Console::WriteLine("SystemWorker 쓰레드가 종료되었습니다. (%d)", std::this_thread::get_id());
}
