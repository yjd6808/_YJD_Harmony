...
void SendFn::SendRoomInfoAck(Room* room, Player* player) {
	...
	if (room == nullptr) {
		...
		strcpy_s(pLoadRoomInfoAck->Reason, REASON_LEN, "참가한 방 정보가 없습니다.");
	} else {
...
void SendFn::SendUpdateCharacterInfoAck(Player* player) {
	...
	if(spQueryResult->GetResultRowCount()) {
		...
		strcpy_s(pUpdateCharacterInfoAck->Reason, REASON_LEN, "캐릭터 정보가 없습니다. account_uid, channel_uid, character_uid mismatch");
	}
...
void SendFn::SendUpdateRoomListAck(Player* player, int channelUID) {
	...
	if (pChannel == nullptr) {
		...
		strcpy_s(pUpdateRoomListAck->Reason, REASON_LEN, "채널 정보가 이상합니다.");
	} else {
