...
create table t_character (
	c_character_id				int				primary key auto_increment			comment '캐릭터 PK',
    c_account_id				int				not null							comment '소유 계정 FK',
...
create table t_attribute (
	c_attribute_id				int				primary key auto_increment			comment '아이템 속성 PK',
...
create table t_item (
	c_item_id					int				primary key auto_increment			comment '아이템 PK',
    ...
    constraint t_item_fk_character_id foreign key (c_character_id) references t_character(c_character_id) on delete cascade,
    constraint t_item_fk_attribute_id foreign key (c_attribute_id) references t_attribute(c_attribute_id) on delete cascade
);
...
create table t_invitation (
	c_invitation_id				int				primary key auto_increment			comment '초대 PK',
    c_invitation_type			tinyint			not null							comment '초대 종류',
    c_requested					timestamp		default current_timestamp			comment '초대한 날짜',
    c_host_character_id			int				not null							comment '초대요청자 FK',
    c_guest_character_id		int				not null							comment '초대수신자 FK',
    
    -- 요청보낸 사람의 캐릭터가 삭제되면 초대도 없어지도록 하는게 맞겠지?
    constraint t_invitation_fk_host_character_id foreign key (c_host_character_id) references t_character(c_character_id) on delete cascade
);

-- [친구 관계 테이블] t_invitationt_friendship
-- 수락, 요청자 합쳐서 PK
create table t_friendship (
    c_req_character_id			int			  	not null							comment '초대요청자 FK',
    c_ack_character_id			int			  	not null							comment '초대수락자 FK',
    c_matched					timestamp		default current_timestamp			comment '친구맺은날',
    
    constraint t_friendship_pk primary key (c_req_character_id, c_ack_character_id),
    constraint t_friendship_fk_req_character_id foreign key (c_req_character_id) references t_character(c_character_id) on delete cascade,
    constraint t_friendship_fk_ack_character_id foreign key (c_ack_character_id) references t_character(c_character_id) on delete cascade
);
