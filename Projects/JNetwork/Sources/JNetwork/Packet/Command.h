...
#define CMD_FUNC_DEF_CONSTRUCT_1(struct)                   static void _Construct(void* _p, int count = 0) { (void)count; return JCore::Memory::PlacementNew(*static_cast<struct*>(_p)); }
#define CMD_FUNC_DEF_CONSTRUCT_2(struct, countable_elem_type)                                                                                                     \
	static void _Construct(void* _p, int count)                                                                                                                   \
	{                                                                                                                                                            \
		JCore::Memory::PlacementNew(*static_cast<struct*>(_p), count);                                                                                            \
...
#define DYNAMIC_CMD_ADD_ZERO_SIZE_ARRAY_FIELD(elem_name)                  \
	...
	{                                                                                                                                                      \
		...
		{                                                                                                                                                  \
			DebugAssertMsg(false, "%s 커맨드의 다이나믹 필드 갯수가 %d인데 %d인덱스에 접근을 시도했습니다.", _Name(), count_, _index);                       \
