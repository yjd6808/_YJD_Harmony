...
#define JNET_PACKET_POOLING_RELEASE(class_name)                                      \
	...
	{                                                                                \
		...
		JCore::MemoryPoolAbstractPtr spGuard = allocator_;                            \
		JCore::MemoryPoolAbstract* pAllocator = allocator_.GetPtr();                  \
		this->~class_name();                                                         \
		pAllocator->DynamicPush(this, sizeof(TPacket));                              \
	}


class IPacket : public JCore::RefCountObject
{
public:
	IPacket() = default;

	explicit IPacket(const JCore::MemoryPoolAbstractPtr& _allocator)
...
protected:
	JCore::MemoryPoolAbstractPtr allocator_; // 소멸 시 반환될 메모리풀
};
...
public:
	...
	RawPacket(const JCore::MemoryPoolAbstractPtr& _allocator, char* _pData, int _length)
	: IPacket(_allocator)
	{
		buf_.buf = _pData;
		buf_.len = _length;
...
struct CommandPacket : IPacket
{
	...
	explicit CommandPacket(const JCore::MemoryPoolAbstractPtr& _allocator)
	: IPacket(_allocator)
	{
	}
	...
	CommandPacket(const JCore::MemoryPoolAbstractPtr& _allocator, CmdCnt_t _commandCount, PktLen_t _packetLength)
	: IPacket(_allocator)
	, commandCount_(_commandCount)
	, packetLength_(_packetLength)
	{
	...
	void ForEach(const JCore::Action<ICommand*>& _consumer)
	{
		int commandIndex = 0;
		char* pCommandData = GetCommandSource();
...
class StaticPacket : public CommandPacket
{
	...
	static_assert(JCore::IsMultipleDerived_v<ICommand, CommandArgs...>,
	              "... CommandArgs must be derived type of \"ICommand\"");

	template <int Index>
	using TypeAt = JCore::IndexOf_t<Index, CommandArgs...>;
...
public:
	...
	explicit StaticPacket(const JCore::MemoryPoolAbstractPtr& _allocator)
...
{
	static_assert(sizeof...(CommandArgs) > 0, "... Packet must have one more command");
	static_assert(JCore::IsMultipleDerived_v<DynamicCommand, CommandArgs...>,
...
public:
	// Ints가 모두 정수형일 때만 동작
	template <typename... Ints, typename = JCore::DefaultEnableIf_t<JCore::And_v<
		JCore::IsIntegerType_v<JCore::IndexOf_t<0, Ints...>>, JCore::IsAllUnaryTrue_v<JCore::Detail::IsIntegerType, Ints...>>>>
	explicit DynamicPacket(Ints... _counts)
	: DynamicPacket(nullptr, _counts...)
	{
	}

	template <typename TAllocator, typename... Ints, typename = JCore::DefaultEnableIf_t<JCore::And_v<
		          std::is_constructible_v<JCore::MemoryPoolAbstractPtr, TAllocator>, JCore::IsAllUnaryTrue_v<JCore::Detail::IsIntegerType, Ints...>>>>
...
public:
	...
	CommandBufferPacket(const JCore::MemoryPoolAbstractPtr& _allocator, const CommandBufferPtr& _buffer);
...
public:
	...
	{
		(void)_count;
		JCore::Memory::PlacementNew(cmd_);
	}

	SinglePacket(const JCore::MemoryPoolAbstractPtr& _allocator, int _count)
