...
{
#ifdef DebugMode
	DebugAssertMsg(_pBuffer->IsValid(), "보내고자하는 커맨드 버퍼 데이터가 이상합니다.");
#endif
...
bool Session::SendToAsync(IPacket* _pPacket, const IPv4EndPoint& _destination)
{
	...
	if (result == SOCKET_ERROR)
	{
		...
		if (errorCode != WSA_IO_PENDING)
		{
			DebugAssertMsg(false, "SendToAsync 실패 (%d)", errorCode);
			_pPacket->Release();
			pOverlapped->Release();
			return false;
		}
...
bool Session::RecvFromAsync()
{
	...
	if (result == SOCKET_ERROR)
	{
		...
		if (errorCode != WSA_IO_PENDING)
		{
			DebugAssertMsg(false, "RecvFromAsync 실패 (%d)", errorCode);
