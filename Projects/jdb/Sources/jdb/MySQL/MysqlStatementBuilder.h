...
public:
	...
	struct Setter<const char[Size]>
	{
		...
		{
			_pArgs[_idx] = MysqlStatementBuilder::ms_pConn->EscapeString(jc::String(_pStr));
		}
	};
	...
	struct Setter<jc::String>
	{
		...
		{
			_pArgs[_idx] = MysqlStatementBuilder::ms_pConn->EscapeString(_val);
		}

		void operator()(jc::String* _pArgs, int _idx, jc::String&& _val) const
		{
			_pArgs[_idx] = Move(MysqlStatementBuilder::ms_pConn->EscapeString(_val));
		}
	};
	...
	template <typename T>
	constexpr static void Set(jc::String(&_fieldMap)[JdbUtil::MAX_ARGS], int _idx, T&& _arg)
	{
	...
	constexpr static void Set(jc::String(&_fieldMap)[JdbUtil::MAX_ARGS], int _idx, T&& _arg, Args&&... _args)
	{
		using TNatural = jc::NaturalType_t<T>;
		Setter<TNatural>()(_fieldMap, _idx, jc::Forward<T>(_arg));
...
public:
	static bool IsInitialized() { return ms_pConn != nullptr; }
