                                  ...
                                  const jc::String& _password, const jc::String& _dbName)
{
	...
	{
		_LogError_(_T("SQLServer ODBC 환경 핸들 할당 실패"));
		return false;
	}
	...
	if (!SQL_SUCCEEDED(ret))
	{
		_LogError_(_T("SQLServer ODBC 버전 설정 실패 : %s"), GetDiagnosticMessage(SQL_HANDLE_ENV, hEnv_).Source());
	...
	if (!SQL_SUCCEEDED(ret))
	{
		_LogError_(_T("SQLServer ODBC 연결 핸들 할당 실패 : %s"), GetDiagnosticMessage(SQL_HANDLE_ENV, hEnv_).Source());
		SQLFreeHandle(SQL_HANDLE_ENV, hEnv_);
		hEnv_ = SQL_NULL_HENV;
		return false;
	}
	...
	_char connStr[512];
	StringUtil::FormatBuffer(connStr, 512,
		_T("DRIVER={ODBC Driver 17 for SQL Server};SERVER=%s,%d;DATABASE=%s;UID=%s;PWD=%s;MARS_Connection=Yes;"),
		hostname_.Source(), static_cast<int>(port_), dbName_.Source(), username_.Source(), password_.Source());

	SQLTCHAR outConnStr[1024];
	SQLSMALLINT outConnStrLen;

	ret = SQLDriverConnect(hDbc_, NULL, (SQLTCHAR*)connStr, SQL_NTS, outConnStr, sizeof(outConnStr) / sizeof(SQLTCHAR), &outConnStrLen, SQL_DRIVER_NOPROMPT);
	...
	{
		isConnected_ = false;
		_LogError_(_T("SQLServer 데이터베이스 연결 실패 : %s"), GetDiagnosticMessage(SQL_HANDLE_DBC, hDbc_).Source());
...
bool SqlServerConnection::SelectDB(const jc::String& _dbName)
{
	if (!isConnected_)
	{
		_LogError_(_T("SelectDB() 실패 : SQLServer 데이터베이스에 연결되어 있지 않습니다."));
		return false;
	}

	jc::String useDbStatement = jc::String(_T("USE ")) + _dbName;

	SQLHSTMT hStmt = SQL_NULL_HSTMT;
	SQLRETURN ret = SQLAllocHandle(SQL_HANDLE_STMT, hDbc_, &hStmt);
	if (!SQL_SUCCEEDED(ret))
	{
		_LogError_(_T("SelectDB() 실패 : 스테이트먼트 핸들 할당 실패"));
		return false;
	}

	ret = SQLExecDirect(hStmt, (SQLTCHAR*)useDbStatement.Source(), SQL_NTS);
	bool success = SQL_SUCCEEDED(ret);

	if (!success)
	{
		_LogError_(_T("SelectDB() 실패 : %s"), GetDiagnosticMessage(SQL_HANDLE_STMT, hStmt).Source());
	}
	else
	{
		dbName_ = _dbName;
...
jc::String SqlServerConnection::GetLastErrorString() const
{
	if (!isConnected_)
	{
		_LogWarn_(_T("GetLastErrorString() 실패 : SQLServer 데이터베이스에 연결되어 있지 않습니다."));
		return _T("연결 안되있음");
	}
...
int SqlServerConnection::GetLastErrorCode() const
{
	if (!isConnected_)
	{
		_LogWarn_(_T("GetLastErrorCode() 실패 : SQLServer 데이터베이스에 연결되어 있지 않습니다."));
		return -1;
	}
...
int SqlServerConnection::GetLastError(OUT jc::String& _str) const
{
	if (!isConnected_)
	{
		_LogWarn_(_T("GetLastError() 실패 : SQLServer 데이터베이스에 연결되어 있지 않습니다."));
		_str = "연결 안되있음";
		return -2;
	}
...
bool SqlServerConnection::BeginTransaction()
{
	if (!isConnected_)
	{
		_LogError_(_T("BeginTransaction() 실패 : SQLServer 데이터베이스에 연결되어 있지 않습니다."));
		return false;
	}
	...
	if (!SQL_SUCCEEDED(ret))
	{
		_LogError_(_T("BeginTransaction() 실패 : %s"), GetDiagnosticMessage(SQL_HANDLE_DBC, hDbc_).Source());
		return false;
	}
...
bool SqlServerConnection::Commit()
{
	if (!isConnected_)
	{
		_LogError_(_T("Commit() 실패 : SQLServer 데이터베이스에 연결되어 있지 않습니다."));
		return false;
	}
	...
	if (!SQL_SUCCEEDED(ret))
	{
		_LogError_(_T("Commit() 실패 : %s"), GetDiagnosticMessage(SQL_HANDLE_DBC, hDbc_).Source());
		return false;
	}
...
bool SqlServerConnection::Rollback()
{
	if (!isConnected_)
	{
		_LogError_(_T("Rollback() 실패 : SQLServer 데이터베이스에 연결되어 있지 않습니다."));
		return false;
	}
	...
	if (!SQL_SUCCEEDED(ret))
	{
		_LogError_(_T("Rollback() 실패 : %s"), GetDiagnosticMessage(SQL_HANDLE_DBC, hDbc_).Source());
		return false;
	}
...
bool SqlServerConnection::SetAutoCommit(bool _enable)
{
	if (!isConnected_)
	{
		_LogError_(_T("SetAutoCommit() 실패 : SQLServer 데이터베이스에 연결되어 있지 않습니다."));
		return false;
	}
	...
	if (!SQL_SUCCEEDED(ret))
	{
		_LogError_(_T("SetAutoCommit() 실패 : %s"), GetDiagnosticMessage(SQL_HANDLE_DBC, hDbc_).Source());
		return false;
	}
...
int SqlServerConnection::GetDiagnostic(SQLSMALLINT _handleType, SQLHANDLE _handle, OUT jc::String& _msg) const
{
	...
	SQLRETURN ret = SQLGetDiagRec(_handleType, _handle, 1, sqlState, &nativeError, messageText, sizeof(messageText) / sizeof(SQLTCHAR), &textLength);
	if (SQL_SUCCEEDED(ret))
	{
		_msg = jc::String((_char*)messageText);
	}
	return nativeError;
}
...
jc::String SqlServerConnection::GetDiagnosticMessage(SQLSMALLINT _handleType, SQLHANDLE _handle) const
{
	...
	if (SQL_SUCCEEDED(ret))
	{
		return jc::String((_char*)messageText);
	}
...
SQLINTEGER SqlServerConnection::GetDiagnosticCode(SQLSMALLINT _handleType, SQLHANDLE _handle) const
{
	...
	SQLGetDiagRec(_handleType, _handle, 1, sqlState, &nativeError, messageText, sizeof(messageText) / sizeof(SQLTCHAR), &textLength);
