...
bool TextButton::init2(float width, float height, const std::string& content, float fontSize) {
	...
	loadTextures(Const::Resource::WhiteRectFileName, "", "", TextureResType::LOCAL);


	const Size btnSize = this->getContentSize();

	m_pBackground = Scale9Sprite::create(Const::Resource::WhiteRectFileName);
	...
	m_pTextUI = Text::create(content, Const::Resource::FontName, fontSize);
	...
	// 버튼의 기능은 이녀석이 수행
	Button* m_pButton = Button::create(Const::Resource::TransparentRectFileName);
