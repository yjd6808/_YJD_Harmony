...
{
	Visitable<JCore::NakedType_t<TQryResultT>>::BindSelectResult(JCore::Forward<TQryResultT>(_v), _query);
}
...
{																														\
	enum Fields { QRY_STRUCT_VARIABLE_NAMES(__VA_ARGS__), Count };														\
																														\
																														\
	constexpr static void BindSelectResult(IN struct_name& v,  MysqlQuerySelect* query) {								\
...
struct SelectStatement
{
	...
	constexpr static void Execute(MysqlDatabase* _database, TResult& _result, Args&&... _args) {
		...
		auto spQuery = _database->Query(TQry::Script, JCore::Forward<Args>(_args)...);
