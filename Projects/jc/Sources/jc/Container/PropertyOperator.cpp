...
void LogLossDataPossible(PropertyType_t _lhs, PropertyType_t _rhs, PropertyBinaryOperatorType_t _operatorType)
{
	...
	_LogWarn_(_T("%hs %hs %hs연산을 수행하였습니다. (데이터가 잘릴 위험이 있습니다.)"), PropertyType::NAME[_lhs],
...
{
	if (!PropertyStatics::EnableOperatorLog) return;
	_LogError_(_T("%hs %hs %hs연산은 수행이 불가능합니다."), PropertyType::NAME[_lhs], PropertyBinaryOperatorType::NAME[_operatorType],
...
{
	if (!PropertyStatics::EnableOperatorLog) return;
	_LogWarn_(_T("%hs %hs %hs연산은 안전하지 않습니다."), PropertyType::NAME[_lhs], PropertyBinaryOperatorType::NAME[_operatorType],
	          PropertyType::NAME[_rhs]);
}
...
{
	if (!PropertyStatics::EnableOperatorLog) return;
	_LogWarn_(_T("%hs %hs %hs연산을 수행하였습니다."), PropertyType::NAME[_lhs], PropertyBinaryOperatorType::NAME[_operatorType],
	          PropertyType::NAME[_rhs]);
}
