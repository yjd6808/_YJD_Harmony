...
struct __Start__
{
}; // 템플릿 파라미터의 시작을 표시하는 용도로 쓰임
struct __End__
{
}; // 템플릿 파라미터의 끝을 표시하는 용도로 쓰임
...
{
	template <template <typename> typename Checker, typename... Args>
	static constexpr bool VALUE()
	{
		...
		{
			return VALUE2<Checker, Args...>();
		}
	}

	template <template <typename> typename Checker, typename T, typename... Args>
	static constexpr bool VALUE2()
	{
		if constexpr (sizeof...(Args) == 0)
		{
		...
		{
			return Checker<T>::VALUE && VALUE2<Checker, Args...>();
		}
	}
};
...
struct Or
{
	static constexpr bool VALUE = Test || Or<RestTests...>::VALUE;
};
...
struct And
{
	static constexpr bool VALUE = Test && And<RestTests...>::VALUE;
};
...
template <>
struct ParameterPackCountOf<__End__>
{
...
template <
	typename... Pack1Args, template <typename...> typename Pack1, 
...
struct IsFunctor
{
	...
	/* 1 */
	template <typename Fn> static constexpr char Test1(SFINAE<decltype(&Fn::operator())>*) { return 1; }
	/* 2 */
	template <typename Fn> static constexpr int Test1(...) { return 1; }
...
struct IsCallable<Binder<Ret, Fx, Types...>> : IntegralConstant<
		bool, std::is_same_v<Binder<Ret, Fx, Types...>, std::_Binder<Ret, Fx, Types...>>>
{
};
...
// jc::Allocator에서 정의된 할당자들이 규칙을 준수해서 제대로 정의되었는지 검사용도
// 함수가 없는 경우도 체크하고싶은데...
...
struct IsValidAllocator
{
	...
	static constexpr bool TestAllocateReturn1()
	{
		return IsPointerType<decltype(TAllocator::template AllocateStatic<Type>())>::VALUE;
	}

	static constexpr bool TestAllocateReturn2()
	{
		return IsPointerType<decltype(TAllocator::template AllocateStatic<Type*>())>::VALUE;
	}

	static constexpr bool TestAllocateReturn3(int _dummy = 0)
	{
		return IsPointerType<decltype(TAllocator::template AllocateDynamic<Type>(_dummy, _dummy))>::VALUE;
	}

	static constexpr bool TestAllocateReturn4(int _dummy = 0)
	{
		return IsPointerType<decltype(TAllocator::template AllocateDynamic<Type*>(_dummy, _dummy))>::VALUE;
	}
	...
	struct AllocateParamCheck<Pack<>> : TrueType
	{
	}; // 인자없는 함수는 void가 아니고 걍 비어있음
	...
	static constexpr bool IS_VALID_ALLOCATE_RETURN_TYPE = 
		TestAllocateReturn1() && TestAllocateReturn2() &&
		TestAllocateReturn3() && TestAllocateReturn4();
	static constexpr bool IS_VALID_ALLOCATE_PARAM_TYPE = TestAllocateParam1() && TestAllocateParam2();


	// ============================================================================

	// DeallocateStatic 함수 리턴 검사
	static constexpr bool TestDeallocateReturn1(void* _dummy = nullptr)
	{
		return IsVoidType_v<decltype(TAllocator::template DeallocateStatic<Type>(_dummy))>;
	}

	static constexpr bool TestDeallocateReturn2(void* _dummy1 = nullptr, int _dummy2 = 0)
	{
		return IsVoidType_v<decltype(TAllocator::template DeallocateDynamic(_dummy1, _dummy2))>;
	}
...
template <template <typename> typename Checker, typename... Args>
constexpr bool IsAllUnaryTrue_v = detail::IsAllUnaryTrue::VALUE<Checker, Args...>();
