...
// @참고2 : https://jacking75.github.io/C++_std_atomic_CompareExchange/ - 한글로 설명된 것(잭킹75)
...
void SpinLock::Lock()
{
	...
	while (!isLocked_.CompareExchange(expected, true))
	{
		// 여기 들어온 경우 이미 다른 쓰레드가 락을 획득한 상태이다.
		//   = 기존 값(isLocked_)이 true이기 때문에 CompareExchange 함수 결과가 false를 반환했다.
		// 
		// 그냥 쌔리마 반복문 계속 돌면서 열때까지 문을 계속 뚜들김

		expected = false;
		// 이걸 해주는 이유가 isLocked_가 true인 경우 expected가 현재 값(true)로 변경되어 버리기 때문에
		// 다시 CompareExchange을 호출 할 때 그냥 무조건 반복문을 탈출해버려서 여러 쓰레드가 
