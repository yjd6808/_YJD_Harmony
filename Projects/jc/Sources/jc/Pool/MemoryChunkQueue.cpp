...
{
	jc_assert_msg(FreeCount() == totalChunkCount_, _T("모도테코나이 메모리 청크가 있습니다."));
...
void CMemoryChunckQueue::Push(void* _pChunk)
{
	...
	jc_assert_msg(freeCount <= totalChunkCount_, _T("알 수 없는 메모리가 스택에 포함되어있는 듯 합니다. Free가 Total보다 많네요..."));
}
