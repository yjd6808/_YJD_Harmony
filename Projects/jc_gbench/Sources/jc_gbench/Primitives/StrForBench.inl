/*
 * StrForBench.inl: 헤더 가드 없음. StrForBench.h에서 전략별 네임스페이스로 반복 include한다.
 * S0는 jc::String의 복사본이다. friend StringUtil은 두지 않는다.
 * (StringUtil의 friend 경로는 공개 API(ExchangeSource/SetLength/SetAtForce)로 대체한다.)
 */
...
public:
	...
	bool EndWith(const StrForBench& _str, bool _caseSensitive = true) const { return FindReverse(_str.Source(), _caseSensitive) == len_ - _str.Length(); }
...
static_assert(sizeof(StrForBench) == 16, "S0 StrForBench size must stay 16 bytes.");
#endif
...
inline StrForBench::StrForBench(const char* _pStr, const int _capacity)
{
	...
	StringUtilT::Copy(pBuffer_, capacity_, _pStr);
}
...
inline void StrForBench::Resize(const int _capacity)
{
	...
	StringUtilT::Copy(pBuffer_, capacity_, pTempBuffer);
...
{
	return StringUtilT::Find(pBuffer_, len_, _startIdx, _endIdx, _pStr, _caseSensitive);
}
...
{
	return StringUtilT::GetRangeUnsafe(pBuffer_, len_, _startIdx, _endIdx);
}
...
inline StrForBench& StrForBench::operator=(const char* _other)
{
	...
	StringUtilT::Copy(pBuffer_, capacity_, _other);
	len_ = iToLen;
