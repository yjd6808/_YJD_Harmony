/*
 * StrForBench_S4.inl: S4 TLS pool + Impl 내부 SSO 후보 (8B 핸들, 64B 슬롯).
 * 헤더 가드 없음. StrForBench.h에서 네임스페이스 s4로 include한다.
 * 짧은 문자열(<=47B)은 pool pop 1회로 끝나고 힙 할당이 없다.
 * move는 포인터 교환이라 공짜이며 Source() 주소가 유지된다.
 */
...
static_assert(sizeof(StrBenchImpl) == 64, "S4 StringImpl slot must stay 64 bytes.");
...
class StrForBench final
{
	...
	static constexpr int SSO_CAP = 47;
...
static_assert(sizeof(StrForBench) == 8, "S4 StrForBench size must stay 8 bytes.");
