/*
 * StrForBench_S2.inl: S2 StringImpl* 후보 (8B 핸들, Impl/버퍼 모두 new/delete).
 * 헤더 가드 없음. StrForBench.h에서 네임스페이스 s2로 include한다.
 * S3~S7의 기준점이 되는 "풀 없음" 바닥 성능을 잰다.
 * pCtx_ == nullptr 상태를 Null/Empty로 쓴다(lazy acquire).
 */
...
private:
	...
	StrBenchImpl& Ctx() { if (!pCtx_) pCtx_ = dbg_new StrBenchImpl{}; return *pCtx_; }
	void FreeCtx()
	{
		if (pCtx_)
		{
...
static_assert(sizeof(StrForBench) == 8, "S2 StrForBench size must stay 8 bytes.");
	...
	: pCtx_(nullptr)
{
	...
	StrBenchImpl* pImpl = dbg_new StrBenchImpl{};
	pImpl->pBuf_ = dbg_new char[expectedCapacity];
	pImpl->cap_ = expectedCapacity;
	pImpl->len_ = length;
	StringUtil::Copy(pImpl->pBuf_, pImpl->cap_, _pStr);
	pCtx_ = pImpl;
}
...
inline void StrForBench::Resize(const int _capacity)
{
	...
	StringUtil::Copy(ctx.pBuf_, ctx.cap_, pTempBuffer);
...
inline StrForBench& StrForBench::operator=(const char* _other)
{
	...
	StringUtil::Copy(Data(), Cap(), _other);
	pCtx_->len_ = iToLen;
