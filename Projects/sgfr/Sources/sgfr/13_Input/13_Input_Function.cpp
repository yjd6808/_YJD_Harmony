 ...
 * =====================
 * 13. 입력 처리 - 보조 함수/셰이더 소스 구현부
 */

#include "Core.h"
#include "sgfr/13_Input/13_Input_Function.h"

using namespace sgf;
using namespace jc;

// 색상 정점을 그대로 통과시키는 HLSL 셰이더 소스를 반환한다. (04번과 동일 구조)
...
{
	printf("\n[입력을 읽는 두 가지 방법]\n");
	printf(" 1. 폴링(Polling): 매 프레임 '지금 눌린 상태인가?'를 직접 묻는다.\n");
	printf("    - IsKeyDown / IsKeyPressed / MouseX / MouseY\n");
	printf("    - 이동처럼 매 프레임 연속적으로 반응할 때 적합\n");
	printf(" 2. 이벤트(Event): '눌리는 순간' 등록해둔 함수가 호출된다.\n");
	printf("    - onKeyPressed / onMousePressed / onMouseWheel 등 (jc::Event 기반)\n");
	printf("    - 버튼 클릭, 메뉴 열기처럼 '발생 순간' 1번만 반응할 때 적합\n");
	printf(" Cocos2d-x의 EventListener가 2번 방식, 유니티의 Input.GetKey가 1번 방식이다.\n");
	printf(" sgf는 둘 다 지원하므로 상황에 맞게 골라 쓰면 된다!\n\n");
}
