...
void PostProcess_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(L"20. 포스트 프로세싱 (0~5 효과, ←→ 경계, ESC 종료)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
	...
	{
		wchar_t szTitle[256];
		swprintf_s(szTitle, L"20. 포스트 프로세싱 - 왼쪽: 원본 | 오른쪽: %s (경계 %d%%) (0~5, ←→, ESC)",
	...
	while (window.PumpMessage())
	{
		...
		device.SetSampler(GraphicDevice::SamplerFilter::Linear, GraphicDevice::SamplerAddress::Clamp, 0);
