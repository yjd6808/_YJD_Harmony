...
void PrintIndexBufferBenefit()
{
	...
	jc::Console::WriteLine(_T(" 사각형 = 삼각형 2개 = 정점 6개가 필요하지만, 실제 꼭짓점은 4개뿐이다."));
	jc::Console::Write(_T(" 인덱스 없이: 정점 6개 x %d바이트 = %d바이트\n"), (_s32)sizeof(sgf::VertexPC), (_s32)(6 * sizeof(sgf::VertexPC)));
	jc::Console::Write(_T(" 인덱스 있음: 정점 4개 x %d바이트 + 인덱스 6개 x 4바이트 = %d바이트\n"),
		(_s32)sizeof(sgf::VertexPC), (_s32)(4 * sizeof(sgf::VertexPC) + 6 * 4));
	jc::Console::WriteLine(_T(" 사각형 하나는 차이가 작지만, 정점 수천 개짜리 모델은 절약이 어마어마해진다!"));
	jc::Console::WriteLine(_T(" (게다가 정점 셰이더 계산 결과도 재활용되므로 속도도 빨라진다)\n"));
}
