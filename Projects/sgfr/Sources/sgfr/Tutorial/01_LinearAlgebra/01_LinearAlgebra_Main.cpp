...
void LinearAlgebra_Main()
{
	...
	jc::Console::Write("right.Dot(diag)       = %.3f (45도: cos45 = 0.707)\n", right.Dot(diag));
	...
	jc::Console::WriteLine("\n[정리]");
	jc::Console::WriteLine(" - 벡터 = 크기 + 방향, 내적 = 방향 유사도, 외적 = 수직 벡터");
	jc::Console::WriteLine(" - 행렬 = 좌표 변환 기계, 곱셈 = 변환 합성 (순서 중요!)");
	jc::Console::WriteLine(" - 우리 엔진은 v' = v * M 규약이라 왼쪽 행렬부터 적용된다");
}
