...
void LinearAlgebra_Main()
{
	...
	PrintVec2(_T("a"), a);
	PrintVec2(_T("b"), b);
	PrintVec2(_T("a + b"), a + b);			// 성분끼리 더하기 = 화살표 이어붙이기
	PrintVec2(_T("a - b"), a - b);			// b에서 a로 향하는 벡터
	PrintVec2(_T("a * 2"), a * 2.0f);			// 길이만 2배 (방향 그대로)

	// 길이: 피타고라스 정리. (3, 4)의 길이는 sqrt(9+16) = 5
	jc::Console::Write(_T("a.Length()            = %.3f (기대값: 5)\n"), a.Length());

	// 정규화: 방향은 그대로, 길이만 1로 만든 벡터. "순수한 방향"이 필요할 때 사용.
	PrintVec2(_T("a.Normalized()"), a.Normalized());
	...
	jc::Console::Write(_T("right.Dot(right)      = %.3f (같은 방향: +1)\n"), right.Dot(right));
	jc::Console::Write(_T("right.Dot(up)         = %.3f (수직: 0)\n"), right.Dot(up));
	jc::Console::Write(_T("right.Dot(-right)     = %.3f (반대: -1)\n"), right.Dot(-right));
	jc::Console::Write(_T("right.Dot(diag)       = %.3f (45도: cos45 = 0.707)\n"), right.Dot(diag));
	...
	PrintMat4(_T("Identity (무변환)"), mat4::Identity());
	PrintMat4(_T("Translation(10, 20, 0)"), mat4::Translation(10.0f, 20.0f, 0.0f));
	PrintMat4(_T("Scale(2, 3, 1)"), mat4::Scale(2.0f, 3.0f, 1.0f));
	PrintMat4(_T("RotationZ(90도)"), mat4::RotationZ(jc_math_deg2rad(90.0f)));

	// 점 변환 확인: (1, 0, 0)을 Z축 90도 회전하면 (0, 1, 0)이 되어야 한다.
	const vec3 p(1.0f, 0.0f, 0.0f);
	PrintVec3(_T("(1,0,0)을 Z축 90도 회전"), mat4::RotationZ(jc_math_deg2rad(90.0f)).TransformPoint(p));
	...
	PrintVec3(_T("회전 -> 이동 (R*T)"), (r * t).TransformPoint(p));	// (10, 1, 0) 부근
	PrintVec3(_T("이동 -> 회전 (T*R)"), (t * r).TransformPoint(p));	// (0, 11, 0) 부근
	...
	jc::Console::WriteLine(_T(" - 행렬 = 좌표 변환 기계, 곱셈 = 변환 합성 (순서 중요!)"));
	jc::Console::WriteLine(_T(" - 우리 엔진은 v' = v * M 규약이라 왼쪽 행렬부터 적용된다"));
}
