...
void PrintCameraExplanation()
{
	...
	jc::Console::WriteLine(_T(" 카메라는 실제로 움직이지 않는다! 세상이 반대로 움직일 뿐이다."));
	jc::Console::WriteLine(_T(" 카메라가 오른쪽으로 3칸 이동 = 모든 물체가 왼쪽으로 3칸 이동"));
	jc::Console::WriteLine(_T(" 그래서 뷰 행렬 = 카메라 이동의 '역변환' 행렬이다."));
	jc::Console::WriteLine(_T("   뷰 = Translation(-카메라위치) x Scale(줌배율)"));
	jc::Console::WriteLine(_T(" 최종 변환: 로컬 -> [월드] -> 월드좌표 -> [뷰] -> 카메라기준 -> [투영] -> NDC\n"));
}
