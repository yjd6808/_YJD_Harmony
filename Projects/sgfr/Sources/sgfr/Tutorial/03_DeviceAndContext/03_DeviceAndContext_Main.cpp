...
{
	jc::Console::WriteLine(_T("=== 03. 디바이스와 컨텍스트 ===\n"));
	jc::Console::WriteLine(_T("[역할 분리]"));
	jc::Console::WriteLine(_T("  ID3D11Device        = 공장   : 버퍼/텍스처/셰이더 같은 리소스를 '만든다'"));
	jc::Console::WriteLine(_T("  ID3D11DeviceContext = 조립라인: 만들어진 리소스를 파이프라인에 '묶고 그린다'\n"));
	jc::Console::WriteLine(_T("[sgf v3 대응]"));
	jc::Console::WriteLine(_T("  GraphicDevice  -> 생성/수명 담당 (Initialize/Finalize, CreateXXX)"));
	jc::Console::WriteLine(_T("  GraphicContext -> 바인딩/그리기 담당 (SetXXX, Draw/DrawIndexed)\n"));
	...
	jc::Console::WriteLine(_T("1) 디바이스 초기화 성공 (창 없이 디바이스만 생성됨)\n"));
	...
	jc::Console::WriteLine(_T("2) 토폴로지를 [삼각형, 삼각형, 선] 순서로 3번 설정했다."));
	jc::Console::Write(_T("   실제 D3D 호출 수   : %llu (예상: 2)\n"), context.GetApiCallCount());
	jc::Console::Write(_T("   캐시로 생략된 호출 : %llu (예상: 1)\n\n"), context.GetSkippedCallCount());

	jc::Console::WriteLine(_T("[정리]"));
	jc::Console::WriteLine(_T("  같은 상태를 다시 설정하면 GraphicContext가 알아서 걸러낸다."));
	jc::Console::WriteLine(_T("  단, 외부에서 원시 컨텍스트를 직접 만졌다면 InvalidateCache()로 캐시를 비워야 한다."));
	jc::Console::WriteLine(_T("  (BeginFrame처럼 엔진 내부에서 상태를 바꾸는 지점이 대표적)\n"));

	// 4. 정리
	g_cResourceMgr.Finalize();
	device.Finalize();
	jc::Console::WriteLine(_T("디바이스 정리 완료. (함수가 반환되면 런처가 목차로 돌아간다)"));
}
