...
void WindowAndMessageLoop_Main()
{
	...
	Window window;
	if (!window.Create(_T("02. 윈도우와 메시지 루프 (ESC로 종료)"), 800, 600))
	{
	...
	{
		jc::Console::Write(_T("[이벤트] 키 누름: %s (코드: 0x%02X)\n"), VkCodeToName(_vkCode), _vkCode);
	});
	...
	input.onMousePressed.Register(1, [](MouseButton _button, _s32 _x, _s32 _y)
	{
		...
		jc::Console::Write(_T("[이벤트] 마우스 %s 버튼 클릭: (%d, %d)\n"), szName, _x, _y);
	});

	jc::Console::WriteLine(_T("윈도우가 생성되었습니다. 아무 키나 마우스를 눌러보세요. (ESC로 종료)"));
	...
	while (window.PumpMessage())
	{
		...
		{
			jc::Console::WriteLine(_T("ESC 입력 -> 종료"));
			break;
		}
