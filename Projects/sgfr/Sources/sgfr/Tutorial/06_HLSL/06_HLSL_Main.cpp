...
void HLSL_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(_T("06. HLSL 셰이더 - 상수 버퍼 애니메이션 (ESC로 종료)"), 800, 600))
	{
	...
	_u64 vsShader = device.Context().CreateVertexShader(jc::StringConvert::FromUtf8(AnimatedShaderSource()));
