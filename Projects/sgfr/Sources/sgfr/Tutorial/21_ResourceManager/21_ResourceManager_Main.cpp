...
void ResourceManager_Main()
{
	...
	jc::Console::WriteLine(_T("--- 1. 디폴트 리소스 ---"));
	jc::Console::Write(_T("초기화 직후 등록 개수: %llu (디폴트 17종)\n"), g_cResourceMgr.GetCount());
	jc::Console::Write(_T("디폴트 텍스처 키: %llu\n"), g_cResourceMgr.GetDefaultTextureKey());
	jc::Console::Write(_T("디폴트 2D 머티리얼 키: %llu\n"), g_cResourceMgr.GetDefaultMaterial2DKey());
	...
	jc::Console::Write(_T("빨간 텍스처 키: %llu, 초록 텍스처 키: %llu\n"), redKey, greenKey);
	...
	jc::Console::Write(_T("Find<Texture>(빨간 키) = %s\n"), (pFound == pRed) ? _T("성공 (같은 포인터)") : _T("실패"));
	jc::Console::Write(_T("Find<Material>(빨간 키) = %hs (타입 불일치 보호)\n"), (pWrongType == nullptr) ? "nullptr" : "???");

	// 경로로 키 역조회
	const _u64 foundByPath = g_cResourceMgr.FindKeyByPath(_T("memory://green"));
	...
	if (pDuplicated != nullptr)
	{
		...
		jc::Console::Write(_T("중복 경로 Add = %llu (0 = INVALID_RESOURCE_KEY, 거부됨)\n"), duplicatedKey);
