 ...
 * =====================
 * 17. 래스터라이저 스테이트 (Rasterizer State)
 *
 * [이 튜토리얼에서 배우는 것]
 *  1. 래스터라이저 단계: 삼각형 -> 화면 픽셀 변환의 옵션 조절
 *  2. 와이어프레임: 메쉬 구조를 선으로 보며 디버깅하는 법
 *  3. 컸링(Culling): 안 보이는 면을 버려 그리기 비용을 절반으로 줄이는 원리
 *  4. 정점 감기 순서(시계/반시계)가 앞면/뒷면을 결정한다는 사실
 *
 * [Before/After 비교 뷰]
 *  같은 큐브가 나란히 두 개 그려진다. (같은 회전, 같은 지오메트리)
 *  - 왼쪽  (Before): 기본 상태 고정 = Solid + Back 컸링
 *  - 오른쪽(After) : 현재 선택한 와이어프레임/컸링 적용
 *  래스터라이저 스테이트도 드로우 단위 상태이므로 드로우 사이에 바꿔 끼워 비교한다.
 *
 * [관찰 포인트]
 *  - Front 컸링으로 바꾸면 오른쪽 큐브만 "속"이 보인다 (뒷면만 남으므로!)
 *  - 와이어프레임 + None 컸링이면 모든 삼각형 모서리가 다 보인다
 *  - 왼쪽 큐브는 무슨 키를 눌러도 그대로다 (기준이니까!)
 *
 * [조작법]
 *  - W: 와이어프레임 토글 (오른쪽에만 적용)
 *  - 1: 컸링 없음 / 2: 뒷면 컸링(기본) / 3: 앞면 컸링 (오른쪽에만 적용)
 *  - ESC: 종료
 */
...
void RasterizerState_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(L"17. 래스터라이저 스테이트 (W 와이어, 1/2/3 컸링, ESC 종료)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
	...
	GraphicDevice::CullMode cullMode = GraphicDevice::CullMode::Back;

	auto UpdateTitle = [&]()
	{
		wchar_t szTitle[256];
		swprintf_s(szTitle, L"17. 래스터라이저 - 왼쪽: Solid+Back(기준) | 오른쪽: %s / %s (W, 1/2/3, ESC)",
	...
	while (window.PumpMessage())
	{
		...
		if (input.IsKeyPressed('1')) { cullMode = GraphicDevice::CullMode::None;      bChanged = true; }
		if (input.IsKeyPressed('2')) { cullMode = GraphicDevice::CullMode::Back;      bChanged = true; }
		if (input.IsKeyPressed('3')) { cullMode = GraphicDevice::CullMode::Front;     bChanged = true; }
		if (bChanged)
		{
			UpdateTitle();
		}
		...
		// [Before] 왼쪽 큐브: 기본 상태 고정 (Solid + Back 컸링)
		DrawCube(-1.1f, false, GraphicDevice::CullMode::Back);
	...
	device.SetCullMode(GraphicDevice::CullMode::Back);
	device.Finalize();
	window.Destroy();
}
