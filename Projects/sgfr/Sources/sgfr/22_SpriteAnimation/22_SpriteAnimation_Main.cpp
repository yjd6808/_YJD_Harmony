 ...
 * =====================
 * 22. 스프라이트 애니메이션 (Sprite Animation)
 *
 * [이 튜토리얼에서 배우는 것]
...
void SpriteAnimation_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(L"22. 스프라이트 - 위: 선형(기준) | 아래: 선택 이징 (1~3, ↑↓ FPS, ESC)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
	...
	{
		wchar_t szTitle[256];
		swprintf_s(szTitle, L"22. 스프라이트 - 위: 선형(기준) | 아래: %s / FPS %d (1~3, ↑↓, ESC)",
	...
	device.SetBlendMode(GraphicDevice::BlendMode::Alpha);
	// 도트 느낌을 살리기 위해 Point 샘플러 (픽셀아트 게임의 핵심 세팅!)
	device.SetSampler(GraphicDevice::SamplerFilter::Point, GraphicDevice::SamplerAddress::Clamp, 0);
