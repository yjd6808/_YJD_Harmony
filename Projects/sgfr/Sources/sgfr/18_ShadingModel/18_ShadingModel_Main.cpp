...
#include "sgfr/18_ShadingModel/18_ShadingModel_Function.h"
...
void ShadingModel_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(L"18. 셰이딩 모델 (1/2/3 모드, ↑↓ 날카로움, ESC 종료)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
	...
	{
		wchar_t szTitle[256];
		swprintf_s(szTitle, L"18. 셰이딩 모델 - %s / 날카로움 %.0f (1/2/3, ↑↓, ESC)",
