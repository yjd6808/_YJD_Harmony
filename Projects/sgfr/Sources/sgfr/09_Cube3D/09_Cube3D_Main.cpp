 ...
 *  꼭짓점은 8개뿐이므로 인덱스 버퍼의 재활용 효과가 크다! (06번 복습)
 *
 * [조작법]
...
void Cube3D_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(L"09. 3D 큐브 - 스페이스바로 깊이 테스트 토글 (ESC로 종료)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
	...
	//    바깥에서 볼 때 시계 방향이 되도록 순서를 맞춰야 한다. (백페이스 컸링)
	const _u32 indices[] = {
		0, 1, 2,  2, 1, 3,	// 앞면   (z-)
		5, 4, 7,  7, 4, 6,	// 뒷면   (z+)
		4, 5, 0,  0, 5, 1,	// 윗면   (y+)
