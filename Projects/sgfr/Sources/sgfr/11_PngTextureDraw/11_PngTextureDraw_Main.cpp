 ...
 * =====================
 * 11. PNG 텍스처 그리기 (PNG Texture) - WIC 디코딩과 UV 좌표계
 *
 * [이 튜토리얼에서 배우는 것]
...
#include "sgfr/11_PngTextureDraw/11_PngTextureDraw_Function.h"
...
void PngTextureDraw_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(L"11. PNG 텍스처 그리기 (ESC로 종료)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
