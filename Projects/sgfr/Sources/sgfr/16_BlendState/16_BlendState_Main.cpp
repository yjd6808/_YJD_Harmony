...
#include "sgfr/16_BlendState/16_BlendState_Function.h"
...
void BlendState_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(L"16. 블렌드 스테이트 (1~4 모드, ESC 종료)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
	...
	GraphicDevice::BlendMode blendMode = GraphicDevice::BlendMode::Alpha;

	auto UpdateTitle = [&]()
	{
		wchar_t szTitle[256];
		swprintf_s(szTitle, L"16. 블렌드 - 왼쪽: Opaque(기준) | 오른쪽: %s (1~4, ESC)",
	...
	while (window.PumpMessage())
	{
		...
		bool bChanged = false;
		if (input.IsKeyPressed('1')) { blendMode = GraphicDevice::BlendMode::Opaque;   bChanged = true; }
		if (input.IsKeyPressed('2')) { blendMode = GraphicDevice::BlendMode::Alpha;    bChanged = true; }
		if (input.IsKeyPressed('3')) { blendMode = GraphicDevice::BlendMode::Additive; bChanged = true; }
		if (input.IsKeyPressed('4')) { blendMode = GraphicDevice::BlendMode::Multiply; bChanged = true; }
		if (bChanged)
		{
			UpdateTitle();
		}
