 ...
 * =====================
 * 04. 렌더링 파이프라인 (Rendering Pipeline) - 화면에 첫 삼각형!
 *
 * [렌더링 파이프라인이란?]
...
#include "sgfr/04_RenderingPipeline/04_RenderingPipeline_Function.h"
...
void RenderingPipeline_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비 (02, 03번에서 배운 내용)
	Window window;
	if (!window.Create(L"04. 렌더링 파이프라인 - 첫 삼각형 (ESC로 종료)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
