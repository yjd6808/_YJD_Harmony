 ...
 * =====================
 * 06. 정점/인덱스 버퍼 (Vertex & Index Buffer) - 사각형 그리기
 *
 * [이 튜토리얼에서 배우는 것]
...
#include "sgfr/06_VertexIndexBuffer/06_VertexIndexBuffer_Function.h"
...
void VertexIndexBuffer_Main()
{
	...
	// 1. 윈도우 + 디바이스 준비
	Window window;
	if (!window.Create(L"06. 정점/인덱스 버퍼 - 사각형 (ESC로 종료)", 800, 600))
	{
		printf("윈도우 생성 실패!\n");
		return;
	}
	...
	// 3. 인덱스 6개 = 삼각형 2개
	//    둘 다 시계 방향이어야 앞면으로 인정된다! (백페이스 컸링 주의)
	const _u32 indices[] = {
		0, 1, 2,	// 삼각형 A
		2, 1, 3,	// 삼각형 B
	};
