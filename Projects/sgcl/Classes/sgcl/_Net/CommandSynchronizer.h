...
class CommandSynchronizer final : public jc::SingletonPointer<CommandSynchronizer>
{
	...
	struct CommandHolder : jc::ObjectPool<CommandHolder>
	{
		...
		: pCommand_(nullptr)
		, pMemPool_(nullptr)
		, listenerType_(ServerType::Max)
		, pSender_(nullptr)
		{
		}

		CommandHolder(ServerType_t _listenerType, jnet::Session* _pSender, jnet::ICommand* _pCopy);
	...
	using IOCPThreadId$CommandQueuePair = jc::Pair<_u32, CommandQueueHolder*>; // IOCP 쓰레드의 ID와 커맨드큐 페어
...
public:
	void EnqueueCommand(ServerType_t _listenerType, jnet::Session* _pSender, jnet::ICommand* _pCmd);
	void Initialize();
