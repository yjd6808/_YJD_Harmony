...
#include "Game/Contents/ActorListenerManager.h"

#include "sgcl/Define/Define_ActorListener.h"

// 캐릭터 리스너
#include "sgcl/Game/Listener/Projectile/ProjectileListener_GunnerBullet.h"

// 몬스터 리스너
#include "sgcl/Game/Listener/Monster/MonsterListener_NormalGoblin.h"
#include "sgcl/Game/Listener/Monster/MonsterListener_DarkGoblin.h"

// 프로젝틸 리스너
#include "sgcl/Game/Listener/Character/CharacterListener_Gunner.h"
...
CharacterListener* ActorListenerManager::CreateCharacterListener(Character* _pCharacter)
{
	...
	if (pFactory == nullptr)
	{
		jc_assert_msg(false, "해당 %sListener를 찾지 못했습니다.", ActorType::Name[ActorType::Character]);
		return nullptr;
	}
...
MonsterListener* ActorListenerManager::CreateMonsterListener(Monster* _pMonster)
{
	...
	if (pFactory == nullptr)
	{
		jc_assert_msg(false, "해당 %sListener를 찾지 못했습니다.", ActorType::Name[ActorType::Monster]);
		return nullptr;
	}
...
ProjectileListener* ActorListenerManager::CreateProjectileListener(Projectile* _pProjectile, Actor* _pSpawner /* = nullptr */)
{
	...
	if (pFactory == nullptr)
	{
		jc_assert_msg(false, "해당 %sListener를 찾지 못했습니다.", ActorType::Name[ActorType::Projectile]);
		return nullptr;
	}
