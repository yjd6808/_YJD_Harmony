...
#include "Game/Action/Gunner/GunnerJump.h"
...
void GunnerJump::ReboundX(Character* _pChar)
{
	...
	default: jc_assert_msg(false, "방향이 도대체 어디에요?");
	}
}
...
void GunnerJump::CreateBullet()
{
	...
	if (isRightFire_)
	{
		...
		case WeaponType::Revolver: iSpawnCode = DEF_PROJECTILE_GUNNER_AUTO_JUMP_RIGHT;
			break;
		default: jc_assert_msg(false, "총 종류가 이상합니다. (1)");
		}
	}
	else
	{
		...
		case WeaponType::Revolver: iSpawnCode = DEF_PROJECTILE_GUNNER_AUTO_JUMP_LEFT;
			break;
		default: jc_assert_msg(false, "총 종류가 이상합니다. (2)");
		}
	}

	jc_assert_msg(iSpawnCode != InvalidValue_v, "프레임 이벤트 ID가 설정되지 않았습니다.");
