...
void ConnectLogin()
{
	...
	pClientGroup->GetLogin()->Connect(IPv4EndPoint{ ServerLoginTcpAddr }, 1000);
}

void ConnectChannel()
{
	// pClientGroup->GetChannel()->ConnectAsync(ServerChannelTcpAddr);
	pClientGroup->GetLogin()->Connect(IPv4EndPoint{ ServerChannelTcpAddr }, 1000);
}

void ConnectGame()
{
	// pClientGroup->GetGame()->ConnectAsync(ServerGameTcpddr);
	pClientGroup->GetLogin()->Connect(IPv4EndPoint{ ServerGameTcpAddr }, 1000);
}
