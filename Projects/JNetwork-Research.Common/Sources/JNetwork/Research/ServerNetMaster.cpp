...
void ServerLoginChannelNetGroup::Initialize()
{
	...
	pLoginServer->Start(IPv4EndPoint{ ServerLoginTcpAddr });

	TcpServerPtr pChannelServer = MakeShared<TcpServer>(pIocp_, pBufferPool_);
	pChannelServer->Start(IPv4EndPoint{ ServerChannelTcpAddr });
...
void ServerGameNetGroup::Initialize()
{
	...
	pGameServer->Start(IPv4EndPoint{ ServerGameTcpAddr });
