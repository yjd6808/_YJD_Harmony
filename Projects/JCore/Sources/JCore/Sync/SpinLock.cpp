...
namespace JCore {
	...
	void SpinLock::Lock() {
		...
		while (!m_bLocked.CompareExchange(expected, true)) {


			// 여기 들어온 경우 이미 다른 쓰레드가 락을 획득한 상태이다.
			//   = 기존 값(m_bLocked)이 true이기 때문에 CompareExchange 함수 결과가 false를 반환했다.
			// 
	...
	bool SpinLock::TryLock() {
		bool expected = false;
		return m_bLocked.CompareExchange(expected, true);
	}
