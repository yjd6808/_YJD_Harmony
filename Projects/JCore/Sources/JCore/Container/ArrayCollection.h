...
protected:
	...
	{
		DebugAssertMsg(this != &_other, "자기 자신에게 대입할 수 없습니다.");
	...
	void CopyElements(T* _pTo, int _toCapacity, T* _pFrom, int _fromCount)
	{
		...
		DebugAssert(_fromCount <= _toCapacity);
	...
	virtual void Expand(int _newCapacity)
	{
		...
		DebugAssertMsg(_newCapacity > capacity_, "현재 용량보다 더 작은 용량입니다.");
		int allocatedSize = 0;
	...
	{
		DebugAssertMsg(IsValidRange(_startIndex, _endIndex),
	...
	{
		DebugAssertMsg(IsValidIndex(_index), "올바르지 않은 데이터 인덱스(%d) 입니다. (컨테이너 크기: %d)", _index, size_);
	...
	void MoveBlock(const int _blockIndex, const int _moveIndex, const int _blockSize)
	{
		...
		// 데이터가 존재하는지
		DebugAssertMsg(IsValidIndex(_blockIndex),
			"(1) 올바르지 않은 데이터 인덱스 입니다. (%d, 컨테이너 크기: %d)", _blockIndex, size_);
		DebugAssertMsg(IsValidIndex(_blockIndex + _blockSize - 1),
			"(2) 올바르지 않은 데이터 인덱스 입니다. (%d, 컨테이너 크기: %d)", _blockIndex + _blockSize - 1, size_);

		// 블록이 이동할 위치가 배열 내부에 둘 수 있는지 체크
		DebugAssertMsg(IsValidIndexCapacity(_moveIndex),
			"(3) 올바르지 않은 데이터 인덱스(%d) 입니다. (컨테이너 크기: %d)", _moveIndex, size_);
		DebugAssertMsg(IsValidIndexCapacity(_moveIndex + _blockSize - 1),
