...
namespace
{
	String MakeFile(const String& _rel, int _bytes)
	{
		const String base = Env::CurrentDirectory();
		const String path = Path::Combine(base, String(_T("test_io/filesvc/")) + _rel);
		{ int slash = path.FindReverse(_T("/")); int bslash = path.FindReverse(_T("\\")); int pos = slash > bslash ? slash : bslash; if (pos > 0) { String parent = path.SubStr(0, pos); Vector<String> segs; String cur; for (int i = 0; i < parent.Length(); ++i) { cur.Append(parent[i]); if (parent[i] == _T('/') || parent[i] == _T('\\') || i == parent.Length()-1) { String dir = cur; while (dir.Length()>0 && (dir.Last()==_T('/') || dir.Last()==_T('\\'))) dir = dir.SubStr(0, dir.Length()-1); if (!dir.IsEmpty() && !Directory::Exist(dir)) Directory::Create(dir); } } } }
		File::Delete(path);
		std::vector<char> body(_bytes > 0 ? _bytes : 1);
		for (int i = 0; i < (int)body.size(); ++i)
			body[(size_t)i] = (char)(i % 251);
		File::WriteAllBytes((_byte*)body.data(), (int)body.size(), StringConvert::ToAnsi(path).Source());
		return path;
	}
	...
	struct IOTestDaemon
	{
		...
		IOTestDaemon()
		{
			...
			fs.Mount(_T("src"), _T("test_io/filesvc/src"));
			fs.Mount(_T("save"), _T("test_io/filesvc/save"));
			fs.SetDefaultAlias(_T("src"));
		}
...
{
	IOTestDaemon env;
	MakeFile(_T("src/table.bin"), 1024);

	MemoryStreamPtr spOut = MakeShared<MemoryStream>(0u);
	IOResult result = env.daemon.Files().Load(_T("src:/table.bin"), spOut);
...
{
	IOTestDaemon env;
	MakeFile(_T("src/table_async.bin"), 1024);

	Atomic<bool> done{ false };
	MemoryStreamPtr spOut = MakeShared<MemoryStream>(0u);
	IOHandle h = env.daemon.Files().LoadAsync(_T("src:/table_async.bin"), spOut,
		[&](const IOResult& r)
	{
		done.Store(true);
		EXPECT_TRUE(r.IsOk());
...
TEST(FileServiceTest, LoadPartialOffsetAndCount)
{
	// P2-1: 오프셋/카운트 부분 읽기
	IOTestDaemon env;
	MakeFile(_T("src/partial.bin"), 1024);

	MemoryStreamPtr spOut = MakeShared<MemoryStream>(0u);
	IOResult result = env.daemon.Files().Load(_T("src:/partial.bin"), spOut, 100, 200);
...
TEST(FileServiceTest, LoadRawBufferOverflowReportsBufferTooSmall)
{
	...
	IOTestDaemon env;
	MakeFile(_T("src/raw.bin"), 512);

	_byte fixed[128];
	IOResult result = env.daemon.Files().Load(_T("src:/raw.bin"), fixed, sizeof(fixed));
...
TEST(FileServiceTest, SaveRoundTripsMemoryToFile)
{
	...
	IOResult saved = env.daemon.Files().Save(_T("save:/roundtrip.bin"), spData);
	EXPECT_TRUE(saved.IsOk());
	EXPECT_EQ(saved.bytesTransferred_, 256u);

	MemoryStreamPtr spOut = MakeShared<MemoryStream>(0u);
	IOResult loaded = env.daemon.Files().Load(_T("save:/roundtrip.bin"), spOut);
...
TEST(FileServiceTest, SaveAsyncCompletesThroughPump)
{
	...
	Atomic<bool> done{ false };
	IOHandle h = env.daemon.Files().SaveAsync(_T("save:/async_save.bin"), spData,
		[&](const IOResult& r)
	{
		done.Store(true);
		EXPECT_TRUE(r.IsOk());
...
TEST(FileServiceTest, LoadTextAndSaveTextHelpers)
{
	...
	IOResult saved = env.daemon.Files().SaveText(_T("save:/hello.txt"), String(_T("hello io")));
	EXPECT_TRUE(saved.IsOk());

	String text = env.daemon.Files().LoadText(_T("save:/hello.txt"));
	EXPECT_TRUE(text == String(_T("hello io")));

	String missing = env.daemon.Files().LoadText(_T("save:/no_such_file.txt"));
...
TEST(FileServiceTest, MissingFileFailsThroughPumpWithListener)
{
	...
	MemoryStreamPtr spOut = MakeShared<MemoryStream>(0u);
	IOHandle h = env.daemon.Files().LoadAsync(_T("ghost.bin"), spOut);
	EXPECT_NE(h, InvalidIOHandle);
...
{
	IOTestDaemon env;
	MakeFile(_T("src/rebind.txt"), 8);

	// 첫 해석 성공
	MemoryStreamPtr spFirst = MakeShared<MemoryStream>(0u);
	IOResult first = env.daemon.Files().Load(_T("rebind.txt"), spFirst);
	EXPECT_TRUE(first.IsOk());

	// 재바인딩: src를 빈 디렉토리로 교체 → 이후 접수분은 미스
	env.daemon.Resolver().Mount(_T("src"), _T("test_io/filesvc/empty_dir"));

	MemoryStreamPtr spSecond = MakeShared<MemoryStream>(0u);
	IOResult second = env.daemon.Files().Load(_T("rebind.txt"), spSecond);
