...
{
	const float rotation = baseInfo_->rotation_ + SGRandom::random_real(baseInfo_->ramdomRotationRangeMin_,
...
{
	DebugAssertMsg(baseInfo_->spawnEffect_, "스폰 이펙트가 없습니다.");
	return baseInfo_->spawnEffect_;
}

EffectInfo* Projectile::GetHitEffectInfo()
{
	DebugAssertMsg(baseInfo_->hitEffect_, "히트 이펙트가 없습니다.");
