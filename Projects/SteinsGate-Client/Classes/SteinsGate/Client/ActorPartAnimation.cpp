...
ActorPartAnimation* ActorPartAnimation::create(
	...
	ActorPartAnimation* pAnimation = dbg_new ActorPartAnimation(animationTarget, animationInfo, frames);
...
void ActorPartAnimation::run(int frameIndexInAnimation) {
	init();
...
void ActorPartAnimation::setPlaySpeed(float speed) {
	m_fPlaySpeed = speed;
}
...
void ActorPartAnimation::updateLoopSequence(float dt) {
...
void ActorPartAnimation::updateAnimation(float currentFrameDelay, FrameTexture* currentFrameTexture, float dt) {
...
void ActorPartAnimation::updateZeroDelayFrame(float currentFrameDelay, FrameTexture* currentFrameTexture) {
...
FrameTexture* ActorPartAnimation::changeTexture(int frameIndexInAnimation) {
	FrameTexture* pFrameTexture = m_vAnimationFrames[frameIndexInAnimation];
...
FrameTexture* ActorPartAnimation::getTexture(int frameIndexInAnimation) {
	FrameTexture* pFrameTexture = m_vAnimationFrames[frameIndexInAnimation];
...
// delay 만큼만 멈춤
void ActorPartAnimation::pauseTime(float delay) {
	m_fPauseDelay = delay;
}
