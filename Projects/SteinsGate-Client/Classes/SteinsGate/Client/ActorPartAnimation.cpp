...
void ActorPartAnimation::constructFrames(int _sgaIndex, int _imgIndex)
{
    ...
    for (int i = 0; i < animationInfo_->Frames.Size(); ++i)
    {
        ...
        if (frameIndex < 0 || frameIndex >= frames_.Size())
        {
            ...
            Core::Contents.PackManager->logTexture("ActorPartAnimation::constructFrames()", { sgaIndex_, imgIndex_, InvalidValue_v }, JCore::LoggerAbstract::eError);
...
{
    DebugAssertMsg(_frameIndexInAnimation >= 0 && _frameIndexInAnimation < animationInfo_->Frames.Size(), "애니메이션 내 프레임 인덱스가 이상합니다.");
