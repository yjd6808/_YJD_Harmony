...
ActorListenerManager::~ActorListenerManager() {
	m_hProjectileListenerMap.ForEachValue([](auto* listener) { delete listener; });
	m_hCharacterListenerMap.ForEachValue([](auto* listener) { delete listener; });
}

void ActorListenerManager::init() {
	// 프로젝틸 리스너 등록
	m_hProjectileListenerMap.Insert(PROJECTILE_LISTENER_GUNNER_BULLET, dbg_new GunnerBulletListener);

	// 캐릭터 리스너 등록
	m_hCharacterListenerMap.Insert(CharType::Gunner, dbg_new GunnerListener);
}
