...
bool ActorSprite::init()
{
	...
	SGVector<ActorPartSpriteData>& partsDataVector = pActorSpriteData_->parts_;

	// 바디 파츠 기준으로 전체 프레임수를 얻는다.
	frameCount_ = ImagePackManager::Get()
	              ->GetPack(partsDataVector[0].sgaIndex_)
	              ->GetSpriteCount(partsDataVector[0].imgIndex_);
...
ActorSprite::PartData ActorSprite::CreatePart(const ActorPartSpriteData& _partSpriteData, int _frameCount)
{
	...
	// 캔버스 위에 파츠를 그린다.
	partData.canvas_ = SGSprite::create();
	...
	partData.boundingBox_ = SGDrawNode::create();
