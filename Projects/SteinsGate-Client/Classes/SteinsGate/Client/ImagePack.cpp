...
ImagePack::ImagePack(const SgaPackagePtr& sgaPackage, int packIndex)
	: m_iIndex(packIndex)
...
int ImagePack::getSpriteCount(int imgIndex) const {
...
SgaSpriteAbstractPtr ImagePack::getSprite(int imgIndex, int frameIndex) const {
...
SgaSpriteAbstractPtr ImagePack::getSpriteUnsafe(int imgIndex, int frameIndex) const {
...
FrameTexture* ImagePack::createFrameTexture(int imgIndex, int frameIndex, bool linearDodge) {
	...
	if (m_TextureCacheMap.Exist(index.Value)) {
		FrameTexture* pCached = m_TextureCacheMap[index.Value];
		return pCached;
	}
	...
	if (sprite.IsLink()) {
		...
		LinkFrameTexture* pLinkTexture = dbg_new LinkFrameTexture(frameIndex, iTargetFrameIndex);
	...
	auto pSpriteTexture = dbg_new SpriteFrameTexture(pTexture, sprite.GetRect(), sprite.GetFrameIndex(), sprite.IsDummy(), linearDodge);
...
void ImagePack::releaseFrameTexture(int imgIndex, int frameIndex) {
...
bool ImagePack::hasImgIndex(const SGString& imgName) const {
