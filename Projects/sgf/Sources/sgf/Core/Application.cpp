 ...
 * 수정일: 8/9/2026 9:30:00 AM (멀티 윈도우 + Renderer3D + 창 표면 분리)
 * =====================
 * 애플리케이션 구현부 (게임 루프)
 */
...
void Application::Run()
{
	...
	while (true)
	{
		...
		// 3.5. OnUpdate에서 메인 윈도우가 닫혔으면 즉시 루프 종료 (파괴된 창으로 BeginFrame 금지)
		if (window_.IsClosed())
		{
			break;
		}
...
void Application::DestroyClosedSubWindows()
{
	for (_s32 i = subWindows_.Size() - 1; i >= 0; --i)
	{
		...
		// 3) 목록에서 제거 (역순 순회라 인덱스 안전)
		subWindows_.RemoveAt(i);
	}
}
