...
bool Window::Create(const jc::String& _title, _s32 _width, _s32 _height)
{
	...
	const DWORD style = WS_OVERLAPPEDWINDOW & ~WS_THICKFRAME & ~WS_MAXIMIZEBOX;	// 크기 고정 창
...
bool Window::CreateSurface(GraphicDevice* _pDevice)
{
	...
	if (!swapChain_.Initialize(pRawSwapChain, _pDevice->Device(), width_, height_))
	{
		return false;
	}

	// 3. 같은 크기의 깊이/스텐실 버퍼를 만든다.
	if (!depthSurface_.Initialize(_pDevice->Device(), width_, height_))
	{
		swapChain_.Finalize();
		return false;
	}
...
LRESULT Window::WndProc(HWND _hWnd, UINT _msg, WPARAM _wParam, LPARAM _lParam)
{
	...
	if (pInput_ != nullptr)
	{
		pInput_->HandleMessage(_msg, _wParam, _lParam);
	}

	switch (_msg)
	{
	case WM_DESTROY:
		// 창이 파괴되었다. closed_만 표시한다 (루프 종료는 PumpMessage가 감지)
		closed_ = true;
		return 0;
