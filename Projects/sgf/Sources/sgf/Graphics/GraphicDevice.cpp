...
bool GraphicDevice::Initialize()
{
	...
	// 2. 상태 객체 캐시 준비
	if (!m_States.Initialize(m_pDevice.Get())) { return false; }

	// 3. 기본 상태 적용 (샘플러 슬롯0, 솔리드+백컸링)
	ID3D11SamplerState* pSamplers[] = {
		m_States.GetSamplerState(SamplerFilter::Linear, SamplerAddress::Clamp)
	};
...
bool GraphicDevice::Initialize(HWND _hWnd, _s32 _width, _s32 _height)
{
	...
	// 2. 깊이/스텐실 버퍼 생성
	if (!m_DepthSurface.Initialize(m_pDevice.Get(), _width, _height)) { return false; }

	// 3. 상태 객체 캐시 준비 (자주 쓰는 상태는 미리 생성)
	if (!m_States.Initialize(m_pDevice.Get())) { return false; }

	// 4. 뷰포트 + 기본 상태 적용 (샘플러 슬롯0, 솔리드+백컬링)
	m_SwapChain.ApplyFullViewport(m_pContext.Get());
...
bool GraphicDevice::CreateDeviceAndSwapChain(HWND _hWnd)
{
	...
	return m_SwapChain.Initialize(pRawSwapChain, m_pDevice.Get(), m_Width, m_Height);
}
...
{
	if (m_pDevice == nullptr || _ppOutSwapChain == nullptr)
	{
		return false;
	}
	...
	return SUCCEEDED(pFactory->CreateSwapChain(m_pDevice.Get(), &scd, _ppOutSwapChain));
}
...
void GraphicDevice::BeginFrame(Window* _pWindow, const Color& _clearColor)
{
	...
	m_pBoundWindow = _pWindow;
	m_Width = _pWindow->Width();
	m_Height = _pWindow->Height();
	...
	m_pContext->ClearRenderTargetView(swapChain.RTV(), clearColor);
	depthSurface.Clear(m_pContext.Get());
}
...
void GraphicDevice::BeginFrame(const Color& _clearColor)
{
	...
	const _f32 clearColor[4] = { _clearColor.r, _clearColor.g, _clearColor.b, _clearColor.a };
	m_pContext->ClearRenderTargetView(m_SwapChain.RTV(), clearColor);

	// 깊이 버퍼는 "가장 멀다(1.0)"로 초기화한다.
	// 그래야 첫 픽셀이 무조건 깊이 테스트를 통과할 수 있다.
	m_DepthSurface.Clear(m_pContext.Get());
}
...
{
	m_pContext->OMSetDepthStencilState(m_States.GetDepthState(_bEnable), 0);
}
...
{
	m_pContext->OMSetBlendState(m_States.GetBlendState(_mode), nullptr, 0xFFFFFFFF);
}
...
{
	ID3D11SamplerState* pSamplers[] = { m_States.GetSamplerState(_filter, _address) };
	m_pContext->PSSetSamplers(_slot, 1, pSamplers);
}
...
{
	if (m_bWireframe != _bEnable)
	{
		m_bWireframe = _bEnable;
		ApplyRasterizerState();
	}
}
...
{
	m_pContext->RSSetState(m_States.GetRasterizerState(m_bWireframe, m_CullMode));
}
...
void GraphicDevice::BindWindowSurface(Window* _pWindow)
{
	...
	m_pContext->OMSetRenderTargets(1, pTargets, depthSurface.DSV());
	swapChain.ApplyFullViewport(m_pContext.Get());
}
...
{
	ID3D11RenderTargetView* pTargets[] = { m_SwapChain.RTV() };
	m_pContext->OMSetRenderTargets(1, pTargets, m_DepthSurface.DSV());
	m_SwapChain.ApplyFullViewport(m_pContext.Get());
}
...
void GraphicDevice::SetRenderTarget(RenderTarget* _pTarget)
{
	...
	m_pContext->PSSetShaderResources(0, 2, pNullSrvs);

	if (_pTarget == nullptr)
	{
		// v2.1: BeginFrame(Window*)로 그리던 중이면 "그 창의 표면"으로 복귀한다.
		if (m_pBoundWindow != nullptr)
		{
			BindWindowSurface(m_pBoundWindow);
			return;
		}
	...
	m_pContext->OMSetRenderTargets(1, pTargets, _pTarget->DSV());
