...
{
	jc_assert_msg(_pCamera != nullptr, _T("기본 카메라가 null입니다."));
...
void Scene::SelectCamera(GameObject* _pCameraObj)
{
	...
	jc_assert_msg(pCamera != nullptr, _T("카메라가 아닌 GameObject를 카메라로 선택할 수 없습니다."));
	if (pCamera != nullptr)
	{
...
void Scene::AddChild(GameObject* _pChild, _u64 _zOrder)
{
	...
	jc_assert_msg(pRunning != nullptr, _T("현재 running 중인 씬이 없습니다. AddChild는 running 씬에서만 가능합니다."));
