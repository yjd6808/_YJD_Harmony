...
using HostStaticCommand = jnet::Command<InterServerCmdType::HostStatic>;
using HostDynamicCommand = jnet::Command<InterServerCmdType::HostDynamic>;
using RelayStaticCommand = jnet::Command<InterServerCmdType::RelayStatic>;
using RelayDynamicCommand = jnet::Command<InterServerCmdType::RelayDynamic>;

template <typename T>
struct IsInterServerCommand : jc::IntegralConstant<bool,
                                                      jc::Or_v<
	                                                      jc::IsBaseOf_v<HostStaticCommand, T>,
	                                                      jc::IsBaseOf_v<HostDynamicCommand, T>,
	                                                      jc::IsBaseOf_v<RelayStaticCommand, T>,
	                                                      jc::IsBaseOf_v<RelayDynamicCommand, T>
                                                      >
	>
{
};
...
template <typename T>
struct IsInterServerHostCommand : jc::IntegralConstant<bool,
                                                          jc::Or_v<
	                                                          jc::IsBaseOf_v<HostStaticCommand, T>,
	                                                          jc::IsBaseOf_v<HostDynamicCommand, T>
                                                          >
...
template <typename T>
struct IsInterServerRelayCommand : jc::IntegralConstant<bool,
                                                           jc::Or_v<
	                                                           jc::IsBaseOf_v<RelayStaticCommand, T>,
	                                                           jc::IsBaseOf_v<RelayDynamicCommand, T>
                                                           >
...
struct __struct__ : HostStaticCommand {                                                           \
	__struct__(int _count = 1) {                                                                   \
		type_ = InterServerCmdType::HostStatic;                                                     \
		cmdId_ = __cmd__;                                                                             \
		cmdLength_ = sizeof(__struct__);                                                               \
	}                                                                                             \
                                                                                                \
...
struct __struct__ : HostDynamicCommand {                                                                            \
	...
	__struct__(int _count) {                                                                                        \
		type_ = InterServerCmdType::HostDynamic;                                                                      \
		cmdId_ = __cmd__;                                                                                               \
...
struct __struct__ : RelayStaticCommand {                                   \
	__struct__(int _count = 1) {                                            \
		type_ = InterServerCmdType::RelayStatic;                              \
		cmdId_ = __cmd__;                                                       \
		cmdLength_ = sizeof(__struct__);                                         \
	}                                                                       \
                                                                            \
