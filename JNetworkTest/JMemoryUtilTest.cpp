...
#include <JMemoryUtil.h>
#include <JType.h>


// 어셈블리어 약간 복습
TEST(JMemoryUtilTest, AssemblyTest) {
...
TEST(JMemoryUtilTest, MemoryCopy) {

	int* c = new int[4];
	int d[4] = { 0, 1, 2, 3};

	JMemoryUtil::Copy(c, sizeof(int) * 4, d, sizeof(d));
...
TEST(JMemoryUtilTest, MemorySet) {
	const int kiArraySize = 32;
	TestStruct* c = new TestStruct[kiArraySize];
	TestStruct d(10);

	JMemoryUtil::Set(c, sizeof(TestStruct) * kiArraySize, 0);
...
TEST(JMemoryUtilTest, MemorySetT) {

	const int kiArraySize = 32;
	TestStruct* c = new TestStruct[kiArraySize];
	TestStruct d(10);

	JMemoryUtil::Set<TestStruct>(c, kiArraySize, d);
