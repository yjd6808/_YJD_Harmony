...
call :build SteinsGate-Tools.UI
if errorlevel 1 exit /b 1

for %%f in ("%BAT_DIR%sgs-*.log") do set "LASTLOG=%%f"
echo [OK] SteinsGate-Tools.UI build completed. [%CONFIG%/%PLATFORM%/MultiByte]
...
echo.
echo ================================================================
echo  [%CONFIG%/%PLATFORM%/MultiByte] Building %~1 ...
echo ================================================================
powershell -ExecutionPolicy Bypass -File "%BUILD_PS1%" -ProjectName %~1 -Configuration %CONFIG% -Platform %PLATFORM% -FileLog -NoConsoleLogging -Multibyte
if errorlevel 1 (
    set "EC=!ERRORLEVEL!"
    echo.
