...
설명은 필수 아님(옵션). 함수명만으로 동작이 자명하면 설명을 생략하고 단일 구분자만 둔다.
설명을 다는 경우 상단 구분자 + 설명줄 + 하단 구분자로 감싼다. 설명줄은 `// : ` 리터럴(콜론+공백)로 시작한다.
하단 구분자와 함수 사이에는 빈 줄을 넣지 않는다.
...
단일 구분자 1줄만 둔다. 구분자와 함수 사이에는 빈 줄을 넣지 않는다.
